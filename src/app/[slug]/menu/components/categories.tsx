...
const RestaurantCategories = ({ restaurant }: RestaurantCategoriesProps) => {
  ...
  const getCategoryVariant = (category: MenuCategoriesWithProducts) => {
      ...
      <ScrollArea className="w-full">
        <div className="flex w-max space-x-4 p-4 pt-0">
          {restaurant.menuCategories.map(category => (
            <Button key={category.id} variant={getCategoryVariant(category)} size="sm" className="rounded-full" onClick={() => handleCategoryClick(category)}>
              {category.name}
            </Button>
          ))}
        </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 flex w-full items-center justify-between border-t bg-white px-5 py-3">
          <div>
            ...
            <p className="text-xs font-semibold">
              ...
              <span className="text-muted-foreground font-normal"> / {totalQuantity} {totalQuantity === 1 ? "item" : "itens"}</span>
            </p>
          </div>
